...
public class PrintFileList {
	...
	public static ArrayList<File> createArray(File directory) {
		...
		/**
		 * Primary and secondary arrays for storage of path-names of files and
		 * directories.
		 * Without secondary array algorithm does not include all path-names of the
		 * files and
		 * directories to the list.
		 */
		ArrayList<File> mainArray = new ArrayList<File>(
				Arrays.asList(fileSpisok));
		...
		while (slider.hasNext()) {
			/** While slider has not passed the entire list... */
			File file = slider.next();
			/**
			 * Get the value of the slider (path-name of the selected element of the
			 * list).
			 */

			if (file.isDirectory()) {
				/** Check whether the value of the slider a directory (subdirectory). */
				int count = slider.nextIndex();
				/** Sent position value of sliders to the "count". */
				slider.remove();
				/** Delete values of slider. */
				mainArray.addAll(Arrays.asList(file.listFiles()));
				/** Content of subdirectory is added to the end of the list. */
				secondaryArray.add(file);
				/** Subdirectory name is recorded at the end of the secondary array. */
				slider = mainArray.listIterator(count - 1);
				/**
				 * Pass the value, which ended, of the runner, with a view of the
				 * completed list.
				 */
	...
	public static void sortAndWrite(ArrayList<File> mainArray) {
		...
		for (int i = 0; i < a; i++) {
			...
			if (file1.isDirectory())
				/** The element of the array is analyzed. */
				arrayOfFiles.add(mainArray.get(i));
			/** If the element is a file, then write it in array of files. */
			else
				arrayOfDirectories.add(mainArray.get(i));
			/** If not - into the array of directories. */
		}
